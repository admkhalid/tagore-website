...
if ( ! class_exists( 'Education_X_About' ) ) {
	...
	class Education_X_About {
		 ...
		 * Single instance.
		 *
		 * @var Education_X_About $instance Instance object.
		 */
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Education_X_About ) ) {
				self::$instance = new Education_X_About;
		...
		public function configure() {
			...
			$this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( 'About %s', 'education-x' ), $this->theme_name );
			$this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( 'About %s', 'education-x' ), $this->theme_name );
			$this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
			$this->page_slug     = $this->theme_slug . '-about';
			$this->page_url     = admin_url( 'themes.php?page=' . $this->page_slug );
			$this->notice        = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme info page.', 'education-x' ), esc_html( $this->theme_name ) ) . '</p><p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'education-x' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-hello="world" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'education_x_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'education-x' ) . '</a></p>';
		}
		...
		public function hooks() {
			...
			// Dismiss AJAX.
			add_action( 'wp_ajax_education_x_dismiss', array( $this, 'dismiss_callback' ) );
			add_action( 'wp_ajax_nopriv_education_x_dismiss', array( $this, 'dismiss_callback' ) );
		}
		...
		public function render_page() {
			?>
			<div class="wrap about-wrap tm-about-wrap">
				...
				<?php endif; ?>
				<a href="https://thememattic.com" target="_blank"><div class="wp-badge"></div></a>
		...
		public function render_current_tab_content() {
			...
			if ( method_exists( $this, $method ) ) {
				...
				printf( esc_html__( '%s() method does not exist.', 'education-x' ), $method );
			}
		}
		...
		public function getting_started() {
			...
			?>
			<div class="feature-section tm-section tm-section-getting-started three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .tm-section -->
			<?php
		}
		...
		public function support() {
			...
			?>
			<div class="feature-section tm-section tm-section-support three-col">
				<?php foreach ( $content as $item ) : ?>
					<?php $this->render_grid_item( $item ); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .tm-section -->
		...
		public function useful_plugins() {
			...
			?>
			<div class="tm-section tm-section-useful-plugins">
				...
				<?php if ( true !== $tgmpa_complete ) : ?>
					<a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'education-x' ); ?></a>
				<?php endif; ?>
		...
		public function demo_content() {
			...
			?>
			<div class="tm-section tm-section-demo-content">
		...
		public function upgrade_to_pro() {
			...
			?>
			<div class="tm-section tm-section-upgrade-to-pro">
		...
		public function assets( $hook ) {
			...
			if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
				wp_enqueue_style( 'education-x-about', get_template_directory_uri() . '/inc/about/css/about' . $min . '.css', array(), '1.0.0' );
				wp_enqueue_script( 'education-x-about', get_template_directory_uri() . '/inc/about/js/about' . $min . '.js', array(), '1.0.0' );
			}
		...
		public function display_admin_notice() {
			...
			$dismiss_status = get_user_meta( $user_id, 'education_x_dismiss_status', true );
			?>
			<?php if ( current_user_can( 'edit_theme_options' ) && 'themes' === $screen_id && 1 !== absint( $dismiss_status ) ) : ?>
				<div class="tm-about-notice notice notice-about">
		...
		public function dismiss_callback() {
			...
			if ( false === wp_verify_nonce( $wpnonce, 'education_x_dismiss_nonce' ) ) {
				wp_send_json( $output );
			}

			update_user_meta( $userid, 'education_x_dismiss_status', 1 );
